...
def build_report(
    ...
    companies = [s for s in core_signals if (s.get("company") or "").strip() and s.get("total_score", 0) >= min_score_targets]
    conn.close()
    ...
    for s in core_signals[:top_n]:
        ...
        if s.get("source_tier") == "secondary_whitelisted":
            lines.append("  - **US relevance (Secondary source):** Basis: " + (s.get("us_relevance_basis") or "") + " | Execution angle: " + (s.get("execution_angle") or ""))
        elif s.get("mining_com_source_page"):
            lines.append("  - **US relevance (Mining.com):** Source page: " + s.get("mining_com_source_page", "") + " | Basis: " + (s.get("us_relevance_basis") or ""))
        else:
    ...
    if sell_ahead:
        for s in sell_ahead[:20]:
            ...
            if s.get("company"):
                cc = s.get("company_country") or "TBD"
                country_suffix = f" — Country: {cc}"
            lines.append(f"- **{s['signal_summary'][:200]}**{company}{country_suffix} — {s['recommended_move']} | {s['posture']} | Score: {s['total_score']} | [Link]({s['link']})")
    else:
    ...
    for c in companies:
        ...
        lines.append(f"- **{name}** – Country: {cc} – {c['recommended_move']} | {c['posture']} (score {c['total_score']}) | {rel_line} – {c['signal_summary'][:100]}... [Link]({c['link']})")
    if not seen:
    ...
    lines.append("- **What it represents:** A heuristic measure of how likely a company-level change will create near- to mid-term business operations or supply chain pain where FP360 services are relevant. Directional, not predictive.")
    lines.append("- **Scoring dimensions:** Operational impact (0–30); Time-to-strain (0–30); FP360 service fit (0–25); Engagement feasibility (0–15).")
...
def write_report(
    ...
    content = build_report(db_path, report_date, reports_dir, top_n, min_score_targets, secondary_whitelist=secondary_whitelist)
